...
export default function ExpensesScreen() {
  ...
  // Load draft on mount
  useEffect(() => {
    const draft = useFormDraftStore.getState().drafts['expense'];
    if (draft) {
      ...
      if (draft.paidByMemberId !== undefined) setPaidByMemberId(draft.paidByMemberId);
      if (draft.date !== undefined) setDate(draft.date);
      if (draft.notes !== undefined) setNotes(draft.notes);
      if (draft.isRecurring !== undefined) setIsRecurring(draft.isRecurring);
    }

    const budgetDraft = useFormDraftStore.getState().drafts['budget'];
    if (budgetDraft) {
      if (budgetDraft.newBudgetLimit !== undefined) setNewBudgetLimit(budgetDraft.newBudgetLimit);
    }
  }, []);
  ...
  const loadData = async (isRefreshing = false) => {
    ...
    try {
      ...
      if (limitSetting && limitSetting.value) {
        ...
        const budgetDraft = useFormDraftStore.getState().drafts['budget'];
        if (!budgetDraft || budgetDraft.newBudgetLimit === undefined) {
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView contentContainerStyle={styles.modalScroll}>
              ...
              <TextInput
                ...
                value={amount}
                onChangeText={(val) => { setAmount(val); setDraft('expense', { amount: val }); }}
              />
              ...
              <View style={styles.pickerContainer}>
                {members.map((m) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { setPaidByMemberId(m.localId); setDraft('expense', { paidByMemberId: m.localId }); }}
                  >
                    <Text style={[
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      if (EXPENSE_SUBCATEGORIES[cat]) {
                        sub = EXPENSE_SUBCATEGORIES[cat][0];
                        setSubcategory(sub);
                      }
                      setDraft('expense', { category: cat, subcategory: sub });
                    }}
                  >
                    <Text style={[
                      styles.pickerChipText,
              ...
              <View style={styles.pickerContainer}>
                {(EXPENSE_SUBCATEGORIES[category] || []).map((sub) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { setSubcategory(sub); setDraft('expense', { subcategory: sub }); }}
                  >
                    <Text style={[
                      styles.pickerChipText,
              ...
              <View style={styles.pickerContainer}>
                {METHODS.map((m) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { setPaymentMethod(m as any); setDraft('expense', { paymentMethod: m }); }}
                  >
                    <Text style={[
                      styles.pickerChipText,
              ...
              <TextInput
                ...
                placeholderTextColor="#868685"
                value={date}
                onChangeText={(val) => { setDate(val); setDraft('expense', { date: val }); }}
              />
              ...
              <TextInput
                ...
                multiline
                value={notes}
                onChangeText={(val) => { setNotes(val); setDraft('expense', { notes: val }); }}
              />
              ...
              <View style={styles.recurringRow}>
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => { const next = !isRecurring; setIsRecurring(next); setDraft('expense', { isRecurring: next }); }}
                >
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput
              ...
              onChangeText={(val) => { setNewBudgetLimit(val); setDraft('budget', { newBudgetLimit: val }); }}
            />
